...
function recordAttempt(ip) {
  ...
  if (now - entry.first > WINDOW_MS) { entry.count = 1; entry.first = now; }
  else { entry.count += 1; }
...
export default async function handler(req, res) {
  ...
  if (typeof password !== 'string') { recordAttempt(ip); return res.status(400).json({ error: 'Missing password' }); }

  const ADMIN = process.env.ADMIN_PASSWORD;
  if (!ADMIN) return res.status(500).json({ error: 'Server misconfigured' });

  try {
    // HMAC + timingSafeEqual for constant-time compare
